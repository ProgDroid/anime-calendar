...
- **There is exactly ONE Redis `SUBSCRIBE` connection per process**, and it is opened lazily on the first `RedisPubSub::subscribe`. `redis_pubsub.rs` holds a `PubSub::split()` sink/stream pair; a single driver task demuxes on `Msg::get_channel_name()` into the per-channel `broadcast` map. Do **not** call `Client::open` + `get_async_pubsub()` again to "keep a channel isolated" — that is what made the budget grow as `2/calendar + 1/viewer` before Task 16. Adding a channel is `sink.subscribe`, removing one is `sink.unsubscribe`; the connection stays up either way, because every other channel is riding on it. Two consequences that bite: **a disconnect now costs every channel**, so `reconnect()` must re-`SUBSCRIBE` the whole live set read from the `subscribers` map (that map is the authoritative record, not a cache); and **lock ordering is `subscribers` before `sink`** everywhere, or `subscribe()` and the reconnect path deadlock.
