...
## Step 5 — backend DRY + quality cleanup batch (2026-06-11 — fully shipped)

The remaining two HIGHs plus their sibling quality items, closed in one cohesive `refactor` commit (the H-17 rename interleaves with M-29/M-30 at the hunk level in `mappers/calendar.rs` and `controllers/stripe_webhook.rs`, so clean per-finding splits weren't possible without interactive hunk staging). Pure refactors — **no behavior change and no SQL text change**, so no `.sqlx` regeneration was needed (which sidesteps the dev-DB migration checksum drift).

- **H-17** — all 37 mapper `*_with` helpers renamed to `*_in_tx` across 16 files (controllers `calendar`/`stripe_webhook`/`user`; mappers `calendar`/`calendar_editor`/`calendar_invitation`/`email_verification`/`password_reset`/`refresh_token`/`subscription`/`user`/`user_settings`; services `entitlement`/`invitation_service`/`sharing_authz`; `test_helpers`). The convention drift (the `_with` surface was larger than the compliant `_in_tx` one) is gone. `services/ics_export.rs`'s std `starts_with`/`ends_with` correctly untouched (the only `*_with` false-positives in the crate).
- **H-18** — extracted `EmailService::send_plain_email(to, subject, body)`; the 4 senders (`send_password_reset`/`send_verification_email`/`send_invitation`/`send_editor_restored`) collapse to an empty-host WARN guard + subject/body + one delegating call (~150 LoC of from/to-parse + message-build + transport boilerplate removed). First test for the previously-untested module: `senders_noop_when_smtp_host_unconfigured` exercises the shared empty-host fast path on all four.
- **M-29** — `suspend_owner_sharing_in_tx` / `restore_owner_sharing_in_tx` now return `ServerResult<Vec<_>>` instead of `Result<_, String>`; the `.map_err(|e| format!(...))` context wrappers collapse to `?` (typed `Error` via `#[from] sqlx::Error`). The 5 `stripe_webhook` call sites — whose handlers keep their own legacy `String`-error chain (a separate, out-of-scope concern) — bridge with `.map_err(|e| e.to_string())?`; reconcile's two log lines switched to `{e:?}` to preserve sqlx detail server-side.
- **M-30** — the 4th calendar-fetch copy (`controllers/sharing.rs::load_calendar_any_owner` — raw SQL in a controller, a layering violation) deleted; replaced by new `CalendarMapper::get_by_id_any_owner_from_pool` (pool→conn adapter delegating to `get_by_id_any_owner_in_tx`). 8 call sites repointed (6 in `sharing`, 2 in `sse`). Calendar-fetch SQL now lives only in the mapper layer.

Verification (sequential): `cargo check -p server --lib --tests` clean · `cargo test -p server --lib` **370 passed / 1 failed** — the single failure is the Redis-gated `subscribe_feed_free_no_blob_no_history_returns_404` panicking at `cache.rs:31` `.expect("test Redis must be reachable")` because no Redis is listening on :2435 in this environment (infra precondition, not a regression; verified the new email test is among the 370 green) · `cargo clippy -p server --lib --tests` **zero new warnings** (the 11 `&mut *tx` deref warnings in `sharing.rs`'s test module are pre-existing — verified identical on a clean `main` via stash, just shifted from lines 225-248 to 198-221 because the M-29 dedup shrank the file ~27 lines). No frontend touched.

Remaining backlog: F2 perf/DB cleanup (F2-22/23/24/25), F2-28 frontend store hygiene, F2-31/F2-32/F2-33, M-* spread items, and the L-* nits.

## Step 6 — perf + DB + cheap-correctness batch (2026-06-12 — fully shipped)

6 findings closed. Pure perf / DDL / dead-code / status-code work — **no `sqlx::query!` text changed**, so no `.sqlx` regeneration (sidesteps the dev-DB checksum drift, same as Step 5).

- **F2-22** — `CachedAnilist::get_items` no longer does 2×N serial Redis GETs. New `Cache::mget<T>(&[String])` helper (single MGET round-trip, per-key hit/miss metrics, empty-input short-circuit); `get_items` issues two MGETs (all meta keys + all airing keys) and zips them index-aligned. Behaviour preserved exactly: both keys must hit; an MGET error degrades to refetch-all (empty Vec → all ids missing), matching the old per-key `unwrap_or(None)`.
- **F2-23** — the PUT pre-check looped `assert_can_add_show` per item, each re-querying `effective_tier` (a `find_active_for_user` round-trip). Added `EntitlementService::assert_can_add_shows` (+ `_in_tx`) which resolve the tier **once** (Paid → skip the whole batch; Free → per-item cap check via the extracted `assert_free_show_cap` / `_in_tx` helpers). Both controller call sites (fast-path + advisory-locked authoritative loop) switched to the batch form. New `assert_can_add_shows_batch_blocks_free_passes_paid` test. Single-item `assert_can_add_show[_in_tx]` retained for `add_item`.
- **F2-24 + F2-25** — migration `20260612000000_username_partial_unique_drop_redundant_indexes.sql` (DDL-only). **F2-24:** `users.username` converted from full `UNIQUE` to partial unique `idx_users_username_active ... WHERE deleted_at IS NULL` (mirrors the email/subscription_token soft-delete shape from 20260412), so a soft-deleted account frees its handle. Verified safe: **no code looks a user up by username** (all lookups are by id/email, both already `deleted_at IS NULL`-filtered) → no row-ambiguity; an active duplicate is still rejected. **F2-25:** dropped `idx_calendar_items_calendar_id` (left-prefix of the PK `(calendar_id,item_id)` and of `idx_calendar_items_calendar_added`) and `idx_calendars_subscription_token` (shadowed by the `..._active` partial unique). *Migration not yet applied to the dev DB — `sqlx migrate run` is still blocked by the pre-existing checksum drift; apply manually (psycopg2 bypass) as in F2-14, or it lands on the next fresh DB / CI run.*
- **M-15** — removed the dead `name` / `user_avatar` store refs from `stores/auth.ts` (written in `oauthLogin`/`logout`, exposed, but read **nowhere** — grep-confirmed). **Deviated from the audit's literal text** ("remove the localStorage.setItem pair"): the `localStorage` `name`/`avatar` entries are *not* dead — `ProfileTab.vue:31-32,88,120` reads them to render the OAuth user's avatar + name (whose `/user/details` username is empty). Removing the writes would blank the OAuth profile. Kept the localStorage writes, removed only the truly-dead refs; documented why in the store.
- **F2-33** — `Error::CannotHashPassword` / `CannotGenerateAuthToken` moved from `400 Bad Request` to `500 Internal Server Error` in `status_code` (server-side argon2/JWT crypto faults, no caller-fixable detail). No leak (static Display); no test asserted 400 on these paths.
