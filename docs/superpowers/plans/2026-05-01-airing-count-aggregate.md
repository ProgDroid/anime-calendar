...
**Status:** Done (2026-05-01, commits eefd5a5 + 02041f6)
...
- [x] Add `pub airing_count: usize` to `PageCalendar` (line 374) — match `item_count`'s type for consistency.
- [x] Inject `anilist: web::Data<Anilist>` into `get_calendars` handler (line 411). Pattern matches sibling handlers at lines 88, 217, 292, 508.
- [x] After the mapper returns `(calendars, total_count)`:
...
- [x] No new SQL → no `cargo sqlx prepare` needed.
- [x] No cache key changes → no version bump.

### Step 2 — Backend tests

**File:** `server/src/controllers/calendar.rs` (existing `#[cfg(test)] mod tests`)

- [x] Pure helper extracted as `fn count_airing(item_ids: &[Id], items_by_id: &HashMap<Id, &Item>, now: i64) -> usize` so the count logic is testable without standing up Anilist. Unit-test it directly:
  - Empty calendar → 0.
...
- [x] If feasible, an integration test on `get_calendars` end-to-end is nice-to-have but not gating. The pure helper covers the logic; the controller wiring is mechanical.

### Step 3 — Frontend types

**File:** `frontend/src/types/calendar.ts`

- [x] Add `airing_count: number;` to `PageCalendar` interface (line 12-20).
...
- [x] Add computed: `const totalAiring = computed(() => calendars.value.reduce((sum, c) => sum + (c.airing_count ?? 0), 0))`
- [x] In the stats block (lines 18–26), append after `totalItems` (with separator, hidden at 0):
  ```vue
...
- [x] `calendars.tile.airing`: en `"{count} airing"` / pt `"{count} a transmitir"`
- [x] `calendars.stats.airing`: en `"{count} airing this season"` / pt `"{count} a transmitir esta temporada"`
...
- [x] CalendarTile: chip visible when `airing_count > 0`, hidden when 0. Use `data-testid="calendar-tile-airing"`.
- [x] MyCalendarsPage: airing total visible only when at least one calendar has `airing_count > 0`. Use `data-testid="my-calendars-airing"`.
- [x] Existing tests pass with the new field added to fixture calendars (default to 0).

### Step 8 — Verify

- [x] `cargo build` (set `AWS_LC_SYS_PREBUILT_NASM=1` per project instructions)
- [x] Backend tests green
- [x] `cd frontend && npm run lint && npm run test:unit && npm run build` green
- [x] Manual smoke: `/my-calendars` page shows the chip on calendars with future-airing items and the page-level total reflects them.
...
## As built — deltas from plan

- Helper signature ended up as `count_airing(item_ids: &[Id], schedules_by_id: &HashMap<u64, &[Schedule]>, now_secs: u64) -> usize` (keyed on `u64` via `Id::to_int()`, not `Id` directly, and on `&[Schedule]` rather than `&Item`). Avoids importing `Item` into the test module and keeps the helper free of the `common::item` surface.
- `now_secs` is `u64` (cast from `Utc::now().timestamp().max(0)` with `#[allow(clippy::cast_sign_loss)]`) to match `Timestamp::to_int()`'s return type — comparison stays in unsigned domain.
- Boundary case picked: schedule exactly at `now` is **not** counted as future (strict `>`). Locked by `schedule_exactly_at_now_is_not_counted_as_future` test.
- Integration tests in `calendar.rs` now register `Anilist::default()` in app data alongside the existing mappers — required because `get_calendars` extracts `web::Data<Anilist>`.
- Unique-id collection uses `HashSet<u64>` + `Vec<Id>` rather than a single `HashMap` to preserve insertion order for the batched `anilist.get_items` call (not strictly required, but cheap).
